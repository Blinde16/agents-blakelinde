...
# Print openclaw config schema so we can identify the correct model config path
echo "=> OpenClaw config schema (model-related keys):"
openclaw config schema 2>/dev/null | grep -i "model\|llm\|openai\|router\|embed" || true
